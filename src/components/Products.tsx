...
const products: Product[] = [
  ...
  // More products will land here as they ship.
];
      ...
      <div className="container-tight">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {products.map((p) => (
            <article
              key={p.name}
              className="group relative bg-gradient-to-br from-neutral-900/60 to-neutral-900/20 border border-neutral-800 hover:border-accent/40 rounded-2xl p-8 transition-colors"
            >
          ...
          <div className="bg-neutral-900/20 border border-dashed border-neutral-800 rounded-2xl p-8 flex flex-col justify-center items-center text-center min-h-[220px]">
            <div className="text-3xl mb-2">⌛</div>
            <h3 className="text-lg font-display font-semibold text-neutral-300">More on the way</h3>
            <p className="text-sm text-neutral-500 mt-1 max-w-xs">
              Next up: a Telegram-driven scheduling agent + a small-business booking tool.
            </p>
          </div>
        </div>
      </div>
